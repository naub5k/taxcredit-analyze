...
const TaxCreditDashboard = () => {
  ...
  const detailedAnalysis = useMemo(() => {
    ...
    if (analysisData.analysisResults && analysisData.analysisResults.length > 0) {
      ...
      if (employeeData && Object.keys(employeeData).length > 0) {
        ...
        for (let i = 1; i < years.length; i++) {
          ...
          if (changeCount < 0) {
            ...
            const recallRisk = calculateRecallRisk(apiResults, currentYear, Math.abs(changeCount));
            
            decreaseAnalysis.push({
              baseYear: currentYear,
              increaseCount: changeCount, // 음수
      ...
      const combinedResults = [...apiResults, ...decreaseAnalysis].sort((a, b) => 
        parseInt(a.baseYear) - parseInt(b.baseYear)
      );
      
      console.log('🎯 최종 결합 결과 (증가+감소):', combinedResults);
    ...
    for (let i = 1; i < years.length; i++) {
      ...
      // 📉 **감소한 경우 환수 위험 분석**
        ...
        const recallRisk = calculateRecallRisk(detailedResults, currentYear, Math.abs(changeCount));
    ...
    console.log('🎯 최종 detailedResults (모든 변화):', detailedResults);
  ...
  const fetchAnalysisData = useCallback(async (inputBizno: string) => {
    ...
    try {
      ...
      // 샘플 데이터 모드
      if (inputBizno === '1234567890' || inputBizno === '좋은느낌') {
        setTimeout(() => {
      ...
      // 🏢 펜타플로 샘플 데이터 모드
      if (inputBizno === '1018197530' || inputBizno === '펜타플로') {
        setTimeout(() => {
          setAnalysisData(pentaploSampleData);
          setLoading(false);
        }, 1000);
        return;
      }

      // 🏢 김종칠세무회계사무소 샘플 데이터 모드
      if (inputBizno === '1010138752' || inputBizno === '김종칠' || inputBizno === '김종칠세무회계사무소') {
        setTimeout(() => {
          setAnalysisData(kimJongChilSampleData);
          setLoading(false);
        }, 1000);
        return;
      }

      // 🎯 **내장 데이터 생성** - API 없이 자체 완결형
      const mockData = generateMockAnalysisData(inputBizno);
  ...
  // 🎯 **내장 데이터 생성 함수**
  const generateMockAnalysisData = (bizno: string) => {
    ...
    return {
      ...
      employeeData: company.employeeData,
      analysisResults: [],
      summary: { 총계: 15350, 사후관리종료: 8800, 사후관리진행중: 6550, 기간경과미신청: 0 }
    };
  };
            ...
            <div className="flex gap-4 items-end">
              <div className="flex-1">
                ...
                <Input
                  {...{
                    type: "text",
                    placeholder: "1018197530 (펜타플로) 또는 '좋은느낌' 입력",
                    value: bizno,
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-gray-900 mb-2">
          🏢 {analysisData.companyInfo.companyName}
        </h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <div className="font-semibold">{analysisData.companyInfo.bizno}</div>
          </div>
          <div>
            <span className="text-gray-500">지역:</span>
            <div className="font-semibold">{analysisData.companyInfo.region}</div>
          </div>
          <div>
            <span className="text-gray-500">업종:</span>
            <div className="font-semibold">{analysisData.companyInfo.industry}</div>
          </div>
          <div>
            <span className="text-gray-500">분석일:</span>
            <div className="font-semibold">{new Date().toLocaleDateString('ko-KR')}</div>
          </div>
        </div>
      </div>
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <h4 className="font-semibold text-red-800 mb-3 flex items-center gap-2">
                    ...
                    <Badge className="bg-red-100 text-red-800 text-xs">
                      {Math.abs(analysis.increaseCount)}명 감소
                    </Badge>
                  </h4>
                  <div className="space-y-3">
                    <div className="p-3 bg-red-100 border border-red-300 rounded">
                      ...
                      <div className="text-xs text-red-700">
                        • {analysis.baseYear}년에 {Math.abs(analysis.increaseCount)}명의 인원 감소가 발생했습니다.
                      </div>
                      <div className="text-xs text-red-700">
              ...
              <tbody>
                {detailedAnalysis.map((analysis: any, index: number) => {
                  ...
                  if (analysis.changeType === 'increase') {
                    // 1차년도 행
                    rows.push(
                      <tr key={`${analysis.baseYear}-1`} className="hover:bg-gray-50">
                        <td className="border border-gray-300 px-3 py-2 font-semibold" rowSpan={3}>
                          {analysis.baseYear}년<br/>
                          <span className="text-sm text-green-600">+{analysis.increaseCount}명</span><br/>
                          <span className="text-xs text-gray-500">
                            청년등 {analysis.adjustedYouthCount || 0}명<br/>
                            청년외 {analysis.othersCount || 0}명
                          </span>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-center" rowSpan={3}>
                        ...
                        <td className="border border-gray-300 px-3 py-2 text-center font-medium">1차년</td>
                        <td className="border border-gray-300 px-3 py-2 text-center font-semibold">
                          {analysis.baseYear}년
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-right">
                          <span className={analysis.employmentCredit?.year1?.available ? 'text-blue-600 font-semibold' : 'text-gray-400 line-through'}>
                            {formatCurrency(analysis.employmentCredit?.year1?.amount || 0)}
                          </span>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-right">
                          <span className={analysis.socialCredit?.year1?.available ? 'text-green-600 font-semibold' : 'text-gray-400 line-through'}>
                            {formatCurrency(analysis.socialCredit?.year1?.amount || 0)}
                          </span>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-right font-bold">
                          <span className={analysis.employmentCredit?.year1?.available ? 'text-purple-600' : 'text-gray-400 line-through'}>
                            {formatCurrency((analysis.employmentCredit?.year1?.amount || 0) + (analysis.socialCredit?.year1?.amount || 0))}
                          </span>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-xs">
                          {analysis.deadlines?.year1?.deadline?.toLocaleDateString?.('ko-KR') || 'N/A'}
                        </td>
                    ...
                    // 2차년도 행
                    rows.push(
                      <tr key={`${analysis.baseYear}-2`} className="hover:bg-gray-50">
                        <td className="border border-gray-300 px-3 py-2 text-center font-medium">2차년</td>
                        <td className="border border-gray-300 px-3 py-2 text-center font-semibold">
                          {analysis.deadlines?.year2?.year || 'N/A'}년
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-right">
                          <span className={analysis.employmentCredit?.year2?.available ? 'text-blue-600 font-semibold' : 'text-gray-400 line-through'}>
                            {formatCurrency(analysis.employmentCredit?.year2?.amount || 0)}
                          </span>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-right">
                          <span className={analysis.socialCredit?.year2?.available ? 'text-green-600 font-semibold' : 'text-gray-400 line-through'}>
                            {formatCurrency(analysis.socialCredit?.year2?.amount || 0)}
                          </span>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-right font-bold">
                          <span className={analysis.employmentCredit?.year2?.available ? 'text-purple-600' : 'text-gray-400 line-through'}>
                            {formatCurrency((analysis.employmentCredit?.year2?.amount || 0) + (analysis.socialCredit?.year2?.amount || 0))}
                          </span>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-xs">
                          {analysis.deadlines?.year2?.deadline?.toLocaleDateString?.('ko-KR') || 'N/A'}
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-center">
                    ...
                    // 3차년도 행 (고용증대세액공제만)
                    rows.push(
                      <tr key={`${analysis.baseYear}-3`} className="hover:bg-gray-50">
                        <td className="border border-gray-300 px-3 py-2 text-center font-medium">3차년</td>
                        <td className="border border-gray-300 px-3 py-2 text-center font-semibold">
                          {analysis.deadlines?.year3?.year || 'N/A'}년
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-right">
                          <span className={analysis.employmentCredit?.year3?.available ? 'text-blue-600 font-semibold' : 'text-gray-400 line-through'}>
                            {formatCurrency(analysis.employmentCredit?.year3?.amount || 0)}
                          </span>
                        </td>
                        ...
                        <td className="border border-gray-300 px-3 py-2 text-right font-bold">
                          <span className={analysis.employmentCredit?.year3?.available ? 'text-purple-600' : 'text-gray-400 line-through'}>
                            {formatCurrency(analysis.employmentCredit?.year3?.amount || 0)}
                          </span>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-xs">
                          {analysis.deadlines?.year3?.deadline?.toLocaleDateString?.('ko-KR') || 'N/A'}
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-center">
                    ...
                    rows.push(
                      <tr key={`${analysis.baseYear}-decrease`} className="hover:bg-red-50 bg-red-25">
                        <td className="border border-gray-300 px-3 py-2 font-semibold bg-red-50">
                          {analysis.baseYear}년<br/>
                          <span className="text-sm text-red-600">{analysis.increaseCount}명 감소</span><br/>
                          <span className="text-xs text-red-500">
                            🚨 환수 위험
                          </span>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-center bg-red-50">
                          <div className="font-semibold text-red-600">{analysis.increaseCount}명</div>
                          <div className="text-xs text-red-500">인원 감소</div>
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-center font-medium bg-red-50">환수위험</td>
